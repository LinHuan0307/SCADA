...
{
    public partial class LSN_materialCodeInputForm : UIForm
    {
        //输入的批次码
        ...
        public LSN_materialCodeInputForm(
        ...
        private async void materialCodeInputForm_LoadAsync(object sender, EventArgs e)
        {
            ...
            ////获取物料绑定列表
            //获取界面上的SN
            var SN = m_UIManeger.Get_Tb_LeftXianShuSN().Value;
        ...
        private async void Scan_DataReceivedAsync(object sender, SerialDataReceivedEventArgs e)
        {
            try
            {
                ...
                //获取界面上的SN
                var leftSN = m_UIManeger.Get_Tb_LeftXianShuSN().Value;
                //var rightSN = m_UIManeger.Get_Tb_RightXianShuSN().Value;
                //申请绑定物料
                var 物料绑定反馈 = await m_MESApi.BindMaterial(leftSN, Sread);
                //物料绑定反馈 = await m_MESApi.BindMaterial(rightSN, Sread);
                //获取物料绑定状态
                var 申请获取绑定物料列表反馈 = await m_MESApi.MaterialStatusBindQuery(leftSN);
                if (!申请获取绑定物料列表反馈.IsSuccess)
                {
                    m_UIManeger.AppendErrorLog("获取左线束的物料绑定状态列表失败！");
                    return;
